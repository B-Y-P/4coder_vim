...
CONSUME_NEXT_KEYSTROKE_SIG(vim_select_register_consume){
	Vim_Register *result = vim_state.params.selected_reg;
   u8 character = vim_state.params.consume_char;
   u32 append = 0;

   if(0){}
	else if(character == '_'){ result = 0; }
	else if(character == '"'){ result = &vim_registers.unnamed;      }
	else if(character == '/'){ result = &vim_registers.search;       }
	else if(character == '.'){ result = &vim_registers.insert;       }
	else if(character == ':'){ result = &vim_registers.command;      }
	else if(character == '='){ result = &vim_registers.expression;   }
   else if(character == '-'){ result = &vim_registers.small_delete; }
   else if(character == '%'){ result = &vim_registers.file;         }
	else if(character == '0'){ result = &vim_registers.yank;         }
	else if(character == '*'){ result = &vim_registers.system;       }
   else if(character == '+'){ result = &vim_registers.system;       }
	else if(in_range('1', character, '9'+1)){ result = &vim_registers.cycle[character-'1']; }
	else if(in_range('a', character, 'z'+1)){ result = &vim_registers.named[character-'a']; }
	else if(in_range('A', character, 'Z'+1)){ result = &vim_registers.named[character-'A']; append = REGISTER_Append; }
	else{ return; }

   vim_state.params.selected_reg = result;
   if(vim_state.params.selected_reg){
      vim_state.params.selected_reg->flags &= (~REGISTER_Append);
      vim_state.params.selected_reg->flags |= append;
   }
...
vim_process_insert_record(Application_Links *app, Record_Info record, i64 *prev_pos){
   ...
   if(record.single_first != *prev_pos){
      *prev_pos = record.single_first;
      text->size = 0;
   }
