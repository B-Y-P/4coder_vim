...
vim_draw_whole_screen(Application_Links *app, Frame_Info frame_info){
	...
	if(vim_lister_view_id == 0){
		...
		if(vim_show_buffer_peek){
			...
			buffer_point.pixel_shift.y = line_height*entry->cur_ratio*(buffer_get_line_count(app, buffer)+1);
			buffer_point.pixel_shift.y -= rect_height(back_rect);
